# Task: TP-003 — add-local-embedding-support / local-reranking
...
```text
openspec/tasks/TP-003-add-local-embedding-support-local-reranking/
...
Implement the approved capability `local-reranking` for change `add-local-embedding-support`. This task realizes one approved contract slice from the planner proposal, design, and delta spec without inventing new behavior or broadening scope.
...
- Feature-gated local reranking backend
- Local reranking preserves input-to-score mapping
- Instruction-aware Qwen3 reranking
...
- `openspec/changes/add-local-embedding-support/specs/local-reranking/spec.md` — capability requirements and scenarios
...
- [ ] Apply the requested delta for `local-reranking` inside the approved module ownership and edit surface
