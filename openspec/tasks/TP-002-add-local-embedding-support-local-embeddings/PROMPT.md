# Task: TP-002 — add-local-embedding-support / local-embeddings
...
```text
openspec/tasks/TP-002-add-local-embedding-support-local-embeddings/
...
Implement the approved capability `local-embeddings` for change `add-local-embedding-support`. This task realizes one approved contract slice from the planner proposal, design, and delta spec without inventing new behavior or broadening scope.
...
- Feature-gated local embedding backend
- Hugging Face GGUF sourcing for local embeddings
- Accelerator feature passthrough
...
- `openspec/changes/add-local-embedding-support/specs/local-embeddings/spec.md` — capability requirements and scenarios
...
- [ ] Apply the requested delta for `local-embeddings` inside the approved module ownership and edit surface
